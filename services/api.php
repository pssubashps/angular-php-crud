<?php
    ...
    die('Connect Error: ' . $this->mysqli->connect_error);
		...
		private function customer(){	
			...
			if($id > 0){	
				$query="SELECT * FROM questions c where c.id=$id";
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				if($r->num_rows > 0) {
					$result = $r->fetch_assoc();	
					$this->response($this->json($result), 200); // send user details
		...
		private function insertCustomer(){
			...
			$column_names = array('question', 'option1', 'option2', 'option3', 'option4','answer');
			...
			if(!empty($customer)){
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				$success = array('status' => "Success", "msg" => "Question Created Successfully.", "data" => $customer);
		...
		private function updateCustomer(){
			...
			$query = "UPDATE questions SET ".trim($columns,',')." WHERE id=$id";
			if(!empty($customer)){
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
